    ...
    <div className = ' fixed top-0 flex w-full items-start justify-between z-4 '>
      ...
      onMouseEnter={() => {
        navGreenRef.current.style.height = '100%'
         navMenuRef.current.style.fill = 'black'
      }}
      onMouseLeave={() => {
        navGreenRef.current.style.height = '0%'
        navMenuRef.current.style.fill = 'white'
      }}
      className='lg:h-14 h-10 lg:w-[16.5vw] w-[25vw] relative bg-black cursor-pointer'>
        <div ref={navGreenRef} className='h-0 w-full transition-all bg-[#D3FD50] absolute top-0'></div>
        <div className='relative lg:top-1 top-2 lg:left-44 lg:w-12 w-6 left-16'>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" id="IconChangeColor" transform="scale(-1, 1)"> <g> <path fill='none' d="M0 0h24v24H0z" id="mainIconPathAttribute"></path> <path  ref={navMenuRef} fill="white" d="M3 4h18v2H3V4zm0 7h12v2H3v-2zm0 7h18v2H3v-2z" id="mainIconPathAttribute"></path> </g> </svg>
        </div>
      </div>
    </div>
  )
